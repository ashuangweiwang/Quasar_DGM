...
import PolyDataGeometry from "../RnderVtkCore/PolyDataGeometry";
...
class DGMJsonIO {
        ...
        for (var index_j = 0; index_j < Maplayers.length; index_j++) {
          ...
          for (let index_z = 0; index_z < layertiles.length; index_z++) {
            ...
            Tileone.geometry = this.geometryToJson(
              layertiles[index_z].PolyDataGeometry
            );
  ...
  TileFromJson(jsondata) {
    ...
    if (layerstiles[index_z].hasOwnProperty("geometry")) {
      //TemTile.PolyDataGeometry = layerstiles[index_z]["geometry"];//数组结构
      TemTile.PolyDataGeometry = this.geometryFromJson(jsondata["geometry"]);
    }
    //TemTile.PolyDataGeometry = jsondata["geometry"];//数组结构
    console.log("tiles", TemTile.id);
    //layer.AddObject(TemTile)
    return TemTile;
  }
  ...
  TileToJson(Tile) {
    ...
    if (Tile.DataFromDB == 0) {
      ...
      //TemTile.tileinfo = Maplayers[index_j]["tileinfo"][index_z];//tileinfo原先在layer
      //console.log("tileinfo", TemTile.tileinfo)
      jsondata.geometry = this.geometryToJson(Tile.PolyDataGeometry);
      //jsondata.geometry = Tile.PolyDataGeometry;//数组结构
      jsondata.DataFromDB = 1; //<新增加属性>
      jsondata.DataState = 1; //<新增加属性>
      console.log("tiles", TemTile.id);
      //layer.AddObject(TemTile)
  ...
  geometryToJson(PolyDataGeometry) {
    var jsondata = [];
    for (let index = 0; index < PolyDataGeometry.length; index++) {
      //if (PolyDataGeometry[index].DataState != 0) {
      var bufferedGeometryOne = {};

      var bufferedGeometry = new PolyDataGeometry();
      bufferedGeometry = PolyDataGeometry[index];
